...
type Tab = "profile" | "orders" | "addresses" | "settings" | "edit-profile" | "edit-address" | "add-address";
...
export default function AccountDashboard() {
  ...
  const handleReorder = (order: DisplayOrder) => {
    ...
    // Show success toast
    if (reorderFeedbackTimer.current) clearTimeout(reorderFeedbackTimer.current);
    setReorderFeedback(`${addedCount} item${addedCount !== 1 ? "s" : ""} added to cart!`);
    reorderFeedbackTimer.current = setTimeout(() => setReorderFeedback(null), 3000);
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 relative">
      {/* Re-order success toast */}
      {reorderFeedback && (
        <div
          className="fixed top-24 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl border border-white/40 animate-fade-in-up"
          style={{
        ...
        >
          ...
          <span className="text-sm font-semibold">{reorderFeedback}</span>
        </div>
      )}
                      ...
                      <div key={order.id} className="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-stone-50 transition-colors">
                        ...
                        <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end mt-2 sm:mt-0">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 py-1.5 px-4 text-sm font-semibold rounded-full whitespace-nowrap cursor-pointer transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
