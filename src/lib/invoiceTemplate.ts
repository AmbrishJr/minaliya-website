    ...
    <table class="items-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>${formatCurrency((item.pricePerUnit - item.discount) * item.quantity * (item.gstPercent / 100))}<br><span style="font-size:10px;">(${item.gstPercent.toFixed(2)}%)</span></td>
          <td class="text-right">${formatCurrency(item.totalPrice)}</td>
        </tr>
        `).join('')}
        <tr class="totals-row">
          <td colspan="3" class="text-left">Total</td>
          <td>${data.items.reduce((sum, item) => sum + item.quantity, 0).toFixed(2)}</td>
          <td colspan="2"></td>
          <td>${formatCurrency(data.items.reduce((sum, item) => sum + item.discount, 0))}</td>
          <td>${formatCurrency(data.items.reduce((sum, item) => sum + ((item.pricePerUnit - item.discount) * item.quantity * (item.gstPercent / 100)), 0))}</td>
    ...
    <table class="summary-section">
      <tr>
        <td class="summary-left">
          <div class="words-box">
            <div class="words-title" style="text-transform:uppercase;">INVOICE AMOUNT IN WORDS</div>
            <div>${numberToWords(data.grandTotal)} rupees only</div>
          </div>
          <div style="margin-top:20px; line-height:1.6;">
            <div style="text-transform:uppercase;font-weight:700;margin-bottom:4px;">TERMS AND CONDITIONS</div>
        ...
        <td class="summary-right">
          <table class="summary-table">
            <tr>
              ...
              <td class="amount" style="font-weight:700;">${formatCurrency(data.subtotal)}</td>
            </tr>
            <tr>
              <td style="font-weight:700;">Discount</td>
              <td class="amount" style="font-weight:700;">${formatCurrency(data.couponDiscount)}</td>
            </tr>
            ${isIntraState ? `
            <tr>
              <td style="font-weight:700;">SGST@2.50%</td>
              <td class="amount" style="font-weight:700;">${formatCurrency(data.sgst)}</td>
            </tr>
            <tr>
              <td style="font-weight:700;">CGST@2.50%</td>
              <td class="amount" style="font-weight:700;">${formatCurrency(data.cgst)}</td>
            </tr>
            ` : `
            <tr>
              <td style="font-weight:700;">IGST@5.00%</td>
              <td class="amount" style="font-weight:700;">${formatCurrency(data.igst)}</td>
            </tr>
            `}
            <tr>
              <td style="font-weight:700;">Round Off</td>
              <td class="amount" style="font-weight:700;">${formatCurrency(data.roundOff)}</td>
            </tr>
            <tr>
              <td style="font-weight:700; padding:6px 0;">Total Amount</td>
              <td class="amount" style="font-weight:700; padding:6px 0;">${formatCurrency(data.grandTotal)}</td>
            </tr>
            <tr>
              <td style="font-weight:700;">Received</td>
              <td class="amount" style="font-weight:700;">${formatCurrency(data.amountPaid)}</td>
            </tr>
            <tr>
